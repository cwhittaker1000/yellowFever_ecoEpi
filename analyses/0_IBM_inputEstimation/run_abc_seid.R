...
prior_sample <- function() {
  a_beta <- rnorm_truncated(0.02, 0.01)
  a_mu <- sample(mu, 1)
  a_p_obs <- sample(p_obs, 1)
  list(beta=a_beta, gamma=0.083, mu=a_mu, p_obs=a_p_obs)
}

f_R0 <- function(parameters) {
  parameters$beta / parameters$mu * parameters$gamma / (parameters$mu + parameters$gamma) * abc_data$max_monkeys
}
...
ggsave("outputs/abc_prior_predictive_R0_seid.pdf", p)
saveRDS(prior_draws, "data/processed/prior_draws_seid.rds")
...
draws <- draws %>%
  mutate(R0=beta / mu * gamma / (gamma + mu) * abc_data$max_monkeys)
saveRDS(draws, "data/processed/abc_draws_seid.rds")
...
p <- qplot(draws$R0) +
  ...
  ylab("Count") +
  annotate("text", x=10, y=100, label=paste0("R0: ", paste(as.list(qs), collapse="-")))
ggsave("outputs/abc_posterior_predictive_R0_seid.pdf", p)
...
big_df <- posterior_predictive_sims(nreps, model_seid, draws, initial_states,
                                    compartments)

g <- plot_posterior_predictive_check(big_df, abc_data)

ggsave("outputs/abc_posterior_predictive_seid.pdf", g)
